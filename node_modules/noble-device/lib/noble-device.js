...
NobleDevice.prototype.setIrTemperaturePeriodNew = function(period, callback) {
  ...
  //characteristic.write(new Buffer([period]), true, function(error) {
    //console.log('Write period to IR temp data');
    callback();
  //});
};
...
NobleDevice.prototype.enableIrTemperatureNEW = function(serviceUuid, characteristicUuid, data, callback) {
  var characteristic = this._characteristics[serviceUuid][characteristicUuid];
  characteristic.write(new Buffer([control]), true, function(error) {
    console.log('Write 01, to enable IR temp data');
    callback;
  });
...
// ACA hay que enchufar el mecanismo de escritura de Jose
